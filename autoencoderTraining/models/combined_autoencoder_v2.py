...
class image_autoencoder(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Freeze primary autoencoder
        with torch.no_grad():
            posterior = self.encode(inputs)  
            z_original = posterior.sample()  
        
        # pass through inner autoencoder
        z_recon = self.latent_ae(z_original) 
    
        # Latent MSE
        latent_loss = F.mse_loss(z_original, z_recon)
        
        # Pixel loss
        with torch.no_grad():
            reconstructions = self.decode(z_recon) 
        
        # MSE between original and reconstructed images passed through both autoencoders
        ms_ssim_pixel_loss = 1 - self.msssim_loss_fn(inputs, reconstructions)
        
        print(ms_ssim_pixel_loss, latent_loss)
        
        total_loss = latent_loss + ms_ssim_pixel_loss
        self.log("train/latent_loss", latent_loss)
        self.log("train/ms_ssim_pixel_loss", ms_ssim_pixel_loss)
        return total_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val/ms_ssim_pixel_loss", ms_ssim_pixel_loss)
        return {"val_loss": latent_loss + ms_ssim_pixel_loss}
