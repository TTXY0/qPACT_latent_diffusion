...
if __name__ == "__main__":
    ...
    for img_number, image_file in enumerate(image_files):
        ...
        with torch.no_grad():
            ...
            encoded_standard = model.encode(original_image).mean  # (4096,)
            decoded_standard = model.decode(encoded_standard)  

            # truncated reconstruction **
            encoded = model.encode(original_image).mean.flatten()  # (4096,)
            encoded_centered = encoded - encoded_mean  

            # project into Truncated Space
            latent_truncated = torch.matmul(U_k.t(), encoded_centered)  # (2000,)

            # reconstruct Latent Vector
            latent_reconstructed_centered = torch.matmul(U_k, latent_truncated)  # (4096,)
            latent_reconstructed = latent_reconstructed_centered + encoded_mean  

            # reshape
            latent_reconstructed = latent_reconstructed.view(1, 4, 32, 32)  

            # decode
            decoded_truncated = model.decode(latent_reconstructed)  


        mse_standard = F.mse_loss(decoded_standard, original_image).item()
        ssim_standard = ssim(decoded_standard, original_image, data_range=2.0).item()
        metrics['MSE_Standard'].append(mse_standard)
        metrics['SSIM_Standard'].append(ssim_standard)


        mse_truncated = F.mse_loss(decoded_truncated, original_image).item()
        ssim_truncated = ssim(decoded_truncated, original_image, data_range=2.0).item()
        ...
        if img_number < 5:  
            ...
            show_images(original_image, decoded_standard, decoded_truncated, save_path=save_path)
            print(f"Saved comparison image to {save_path}")


    torch.save(metrics, f'error_metrics_k{k}.pt')
