import os
import sys
sys.path.append("/workspace/thomas/latentDiffusion/stable-diffusion")
import yaml
...
image_dir = "/home/twynn5403/openimages/scissors/images/"


image_files = [os.path.join(image_dir, fname) for fname in os.listdir(image_dir) if fname.endswith('.jpg')][:10]
...
for i, img_path in enumerate(image_files):
    ...
    difference_image = torch.abs(reconstruction - original_image).numpy()

    axes[i, 0].imshow(original_image.numpy())
    axes[i, 0].set_title("Original Image")
    axes[i, 0].axis("off")

    axes[i, 1].imshow(reconstruction.numpy())
    axes[i, 1].set_title("Reconstructed Image")
    axes[i, 1].axis("off")

    im = axes[i, 2].imshow(difference_image, cmap='gray', vmin=0, vmax=0.4)
    axes[i, 2].set_title("Difference")
    axes[i, 2].axis("off")

#fig.colorbar(im, ax=axes[:, 2], orientation='horizontal', fraction=0.1)

# Save the figure to a file
plt.savefig("original_and_reconstructed_grid.png")
