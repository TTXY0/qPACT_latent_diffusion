...
from models.working_autoencoder_linear_compression import AutoencoderKL
...
def show_images(original, reconstructions, save_path=None):
    ...
    for col in range(3):
        image = reconstructions[col].squeeze().cpu().numpy()
        ...
        global_min = -5.315805745492602
        global_max = 1.4058262706509637
        image = (image * (global_max - global_min)) + global_min
        im = ax[col].imshow(image, cmap='viridis')#, vmin=global_min, vmax=global_max)
        ax[col].axis("off")
...
if __name__ == "__main__":
    ...
    for img_number in range(3):
        ...
        index += 1
    save_path = f"/workspace/thomas/latentDiffusion/autoencoderTraining/scripts/imgs/autoencoder_recon_{img_number}_oldU_2addglobal.png"
    show_images(original_image, reconstructions, save_path=save_path)
